  ...
  try {
    Bun.serve({
      ...
      async fetch(req) {
        ...
        // Serve the Android project zip directly from shared storage
        if (pathname === "/download/android-project.zip") {
          const zip = Bun.file("/home/team/shared/android-project.zip");
          if (await zip.exists()) {
            return new Response(zip, {
              headers: {
                "Content-Type": "application/zip",
                "Content-Disposition": 'attachment; filename="vitality-compass-android-project.zip"',
              },
            });
          }
